#!/usr/bin/env bash
# This script migrates all VolSync applications to NFS storage classes.
# Usage: ./migrate-all-volsync-apps.sh [-d] [-r]
# Where:
#   -d: Delete PVCs related to the applications
...
# Check if volsync-migrate-to-nfs.sh exists
if [ ! -f "./volsync-migrate-to-nfs.sh" ]; then
  write_error "volsync-migrate-to-nfs.sh not found in the current directory. Exiting."
  exit 1
fi

# Make sure the script is executable
chmod +x ./volsync-migrate-to-nfs.sh
...
echo "$REPLICATION_SOURCES" | jq -r '.items[] | .metadata.namespace + " " + .metadata.name' | while read -r line; do
  ...
  COMMAND="./volsync-migrate-to-nfs.sh -n $NAMESPACE -a $APP"
