...
# Master script to fix VolSync issues
# This script will:
# 1. Fix VolumeSnapshotClass issues
# 2. Clean up stuck resources
# 3. Trigger reconciliation of Flux resources
...
echo -e "${BLUE}Starting VolSync fix process for namespace: ${NAMESPACE}${NC}"
if [ ! -z "$APP_NAME" ]; then
  echo -e "${BLUE}Filtering for app: ${APP_NAME}${NC}"
fi

# Step 1: Fix VolumeSnapshotClass issues
echo -e "${BLUE}Step 1: Fixing VolumeSnapshotClass issues...${NC}"
./fix-volumesnapshotclass.sh
echo -e "${GREEN}VolumeSnapshotClass issues fixed${NC}"

# Step 2: Clean up stuck resources
echo -e "${BLUE}Step 2: Cleaning up stuck resources...${NC}"
./unstuck-volsync.sh ${NAMESPACE} ${APP_NAME}
echo -e "${GREEN}Stuck resources cleaned up${NC}"

# Step 3: Trigger reconciliation of Flux resources
echo -e "${BLUE}Step 3: Triggering reconciliation of Flux resources...${NC}"
...
echo -e "${GREEN}VolSync fix process completed for namespace: ${NAMESPACE}${NC}"
